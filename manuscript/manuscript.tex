...
than $n$. Figure~\ref{fig:block_dist} illustrates the typical behavior by
...
\begin{figure}[tbp]
  ...
  \label{fig:alt_qq_g}
\end{figure}
\jy{Watch for missing figure labels}
...
the selected block size was $\sqrt{n}$ or lower. The size performane is shown
...
nor the cubic root setting holds its size. The power results are shown in
Figure~\ref{fig:alt_rr}. For $|\tau| \leq 0.5$, the two settings exhibit
\jy{comparing with the power figure in the simulation section. Right? Clarify.}
comparable power, with the automatic selection sometimes yielding higher
