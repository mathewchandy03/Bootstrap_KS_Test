...
Theoretically, $K_n(x)$ and the simpler plug--in correction
$C_n(x)=\sqrt{n}\{F_n(x)-F(x;\hat\theta_n)\}$ are asymptotically
equivalent when the series is independent or exhibits only
short--memory dependence.  The joint process of $(F_n,\hat\theta_n)$
converges to a mean--zero Gaussian limit whose long--run covariance
aggregates auto--covariances across lags.  When the circular block
bootstrap reproduces this long--run covariance, the conditional bias of
the plug--in statistic is of smaller order $O_p(l/n)$, where $l$ is the
block length.  The expectation centering in $K_n(x)$ removes this minor
...
through additional simulations; see results in Section~2 in the
...
The comparable results of $K_n(x)$ and $C_n(x)$ suggest that the choice
between them is largely immaterial in practice.  We therefore retain
$K_n(x)$ for theoretical completeness and consistency of presentation,
while acknowledging that either correction performs similarly in finite
samples.
...
additional simulation are provided in Section~2
...
\jy{This inequality does not make sense.}
