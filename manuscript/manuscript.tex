...
setting held its size quite well with small $n$ for $|\tau| \leq 0.25$, whereas
$|\tau| \geq 0.5$ may require $n > 200$, and $|\tau| \geq 0.75$ may 
require $n > 800$.
Similarly, for $|\tau| > 0.5$, the automatic selection
does not hold its size. In addition, the automatic selection actually seems
to have larger deviations from the line for negative $\tau$, which
may be a result of the overly large block sizes. Notably, while the
cubic root setting held its size for $\tau = -0.25$ for small $n$, the automatic
selection does not.
Still, under $\tau = 0$ and $\tau = 0.25$, the method with block size 
selected by
\citet{politis2004automatic} maintains reasonable size for moderately positively
dependent series. 

The power results are shown in
...
size may be more challenging. In such cases, it may be sensible to be more
restrictive when setting the final block size --- for example, using
$\min \{\sqrt{n}, \hat l\}$.
