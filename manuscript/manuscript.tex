...
$l = \lceil n^{1/3} \rceil$ \citep{buhlmann1999block}... \citet{hall1995blocking}
and
...
\begin{enumerate}
\item
  Generate $X^{(b)}_1,...,X^{(b)}_n$ by applying circular block bootstrap
...
\item
  ...
  \[
    T^{(b)}_n = \sup_x \Big\vert \sqrt{n}\left(F^{(b)}_n(x)
    - F(x; \hat\theta^{(b)}_n)\right) - \kappa_n(x) \Big\vert.
  \]
  where
  $\kappa_{n}(x) = \sqrt{n}(\mathbb{E}^{*}[F^{(b)}_n(x)] -
...
When parameters of a hypothesized distribution $F(x;\theta)$ are estimated from 
the data (yielding $\hat\theta_n$), the null distribution of the KS statistic 
...
mean or systematic deviation. The bias correction $\kappa_n(x)$ estimates this mean. 
Specifically, $E^*[F_n^{(b)}(x)]$ is the average empirical CDF over all 
bootstrap samples, and $\theta^*_0 = E^*[\hat\theta^{(b)}_n]$ is the average of 
the parameter estimates from these bootstrap samples. Thus, $\kappa_n(x)$ 
quantifies 
the expected difference, in the bootstrap world, between the empirical CDF and 
the hypothesized CDF (evaluated at the average bootstrap parameter estimate). 
Subtracting $\kappa_n(x)$ from 
$\sqrt{n}(F_n^{(b)}(x) - F(x;\hat\theta^{(b)}_n))$ centers 
the 
...
the original $T_n$ statistic around its true but complex null distribution. 
This approach extends the logic of \citet{babu2004goodness}, who introduced a 
similar correction for independent data to the context of serially dependent 
time series via block bootstrapping.
...
this study. Firstly, there are many other variants of bootstrap besides
circular block bootstrap which
can be used for serially dependent data such as the
stationary bootstrap \citep{politis1994stationary} and the dependent wild
bootstrap \citep{shao2010dependent}. It would be interesting to investigate how
these methods perform using our correction. Secondly, analogous
methodologies for other goodness-of-fit tests
...
often serially dependent. Thirdly, an extension of the proposed NPBB
...
scheme
for
circular block boostrap as described in Section 3.3 of
