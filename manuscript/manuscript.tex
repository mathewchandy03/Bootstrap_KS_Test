...
\jy{What was the value of $\sigma_\epsilon$?}
...
\begin{figure}[tbp]
  ...
  \includegraphics[width = \textwidth]{figures/gamma}
...
\jy{merge the two tables; use bottomrule and toprule instead of hline;
  put them in this file instead of inputing.}
...
\begin{figure}[tbp]
  ...
  \caption{Power curve as a function of $\tau$ for
    ...
    $\in \{N(8,8), \Gamma(8,1)\}$.\jy{clarify the data generating distribution and the
...
\jy{comment on the asymmetry of the power in response to tau.}
...
different dependence structures such as MA, ARMA, and ARIMA
processes. Additionally, one could propose a similar method
for comparing two serially dependent methods. One could also apply the method
showcased in this paper to fields like earthquake prediction and astronomy.
