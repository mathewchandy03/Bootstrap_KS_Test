...
the KS process. However, this bootstrap process inherently possesses
its own mean or systematic deviation. The bias correction $K_n(x)$
estimates this mean.
...
Theoretically, $K_n(x)$ and the simpler correction $C_n(x)$ are
asymptotically equivalent when the series is independent or exhibits
only short-memory dependence.  The joint process of
$\sqrt{n}(F_n, \hat\theta_n)$ converges to an $O_p(1)$
mean-zero Gaussian limit whose long-run covariance
aggregates autocovariances across lags.  When the circular block
bootstrap reproduces this long-run covariance, the conditional bias of
the $\sqrt{n}$-scaled process associated with
$C_n$ is of smaller order $O_p(l/\sqrt{n})$ as long as $l$ is
$o(\sqrt{n})$ \citep{buhlmann1994blockwise, lahiri1999theoretical}.
The expectation centering in $K_n(x)$ removes this minor
bias exactly but does not alter the first-order limit.  Consequently,
both corrections yield the same asymptotic distribution, and differences
appear only through higher-order terms that vanish as the sample size
increases.


To evaluate the finite sample impact, we compared the two corrections
through additional simulations; see results in Section~1 in the
Supplement.  Across all sample sizes and dependence
levels considered, the empirical sizes and powers were nearly identical.
This finding supports the theoretical argument that the circular block
bootstrap already captures the leading effects of serial correlation,
leaving little residual bias for the expectation centering to remove.
The comparable results of $K_n(x)$ and $C_n(x)$ incidate that, under
the examined settings, the difference between the corrections is
small. However, this may not hold under stronger dependence, longer
block lengths, or more complex statistics, where expectation centering
could have a more noticeable effect. We retain $K_n(x)$ for theoretical
completeness and consistency of presentation, while noting that either
correction performs similarly in our simulation settings.
...
additional simulation study are provided in Section~2
