...
\jy{Recover the definition of jy and eds for comments. Prepare two
  definitions in the source so that one can be easily commented out.}
\mc{addressed}

\jy{Cite Kunsch (1989) in the proof as appropriate}
\mc{addressed}

\jy{In the Supp: make sure notations are consistent (e.g., use
  $\{X_t: t = 1, \ldots, n\}$ for observed data);
  clean up the styles (linewidth etc.)}
\mc{addressed}
...
\begin{proposition}\label{prop:Tnboot}
  ...
  based on the bootstrap sample $\{X_t^{(b)}: t = 1, \ldots, n\}$.
\end{proposition}
...
usually larger than 1, which indicates that the test is too liberal. In
