...
\jy{In the supplement, use the proof environment to give proof
  sketches; make the title ``Supplement to `paper title` by
  authors''. No need to use their template for supp.}

Assume that the time series $\{X_t\}$ is stationary and
strongly mixing. The following results provides theoretical validity
of the NPBB procedure of the KS test. Sketches of their proofs are
in the Supplementary Information, with most arguments similar to those
...
\begin{proposition}\label{prop:Tnboot}
  As $1/l+l/n\rightarrow 0$,
  \[
    \big\vert \mathbb{P}(T_n^*\leq x \mid \{X_t: t = 1, \ldots, n\}) -
    \mathbb{P}(T_n\leq x) \big\vert
    \overset{a.s.}{\rightarrow} 0
  \]
  for any fixed $x$, where and $T_n^*$ is the the statistic computed 
  based on the bootstrap sample $\{X_t^*: t = 1, \ldots, n\}$.
\end{proposition}
...
\[
  \sup_x \big \vert
  \mathbb{P}(T_n^*\leq x|X_{1:n}) - \mathbb{P}(T_n\leq x) \big\vert
  \overset{a.s.}{\rightarrow} 0.
\]
