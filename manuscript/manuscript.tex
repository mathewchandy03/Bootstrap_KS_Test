...
distribution $F(x; \hat\theta_n)$ fitted onto $X_t$, and let 
...
\mc{addressed in third paragraph of introduction}
...
(under the $H_0: F(x) = F(x; \theta_0)$ for some true
parameter $\theta_0$.)
...
\jy{Remove the white space in the pdf figures. Could put the zoomed version in
  the same figure.}\mc{addressed}
...
of the p-values moreso than negative serial dependences of the same strength.
...
\jy{Not true from the plots.}\mc{addressed}
uniformly distributed as long as the serial dependence is not too high. 
This is an indication that our method 
holds its size for moderate serial dependence levels.
For such cases, we can expect the probability
of a type 1 error to be approximately equivalent to the significance level.

From Tables~\ref{table:rr_norm} and \ref{table:rr_gamma}, we can observe 
that the rejection rate is close to the significance level
as long as 
Kendall's $\tau$ is less than or equal to 0.5 and a sample size greater
than 100 is used.
...
$-0.9$ to $0.9$. Using a sample size $n \geq 800$ appears to be sufficient
\jy{What are the findings? summarize bullet points before forming texts.}
...
\jy{don't astrophie citet; let alone citep}\mc{addressed}
