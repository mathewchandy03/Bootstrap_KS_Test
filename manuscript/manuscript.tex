...
the KS statistic under $H_0$. Let $\hat{\theta}_0 = 
\mathbb{E}^{(b)}[\hat\theta^{(b)}_n]$. In particular, we
...
$\sqrt{n}(F(x; \hat\theta^{(b)}_n) - F(x; \hat{\theta}_0))$ as
...
$\hat{\theta}_0 = \mathbb{E}^{(b)}[\hat\theta^{(b)}_n]$ can be approximated by, respectively,
