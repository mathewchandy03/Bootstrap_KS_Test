...
Let $f(x; \theta) = \frac{\partial F(x; \theta)}{\partial \theta}$,
for some hypothesized distribution $F(x; \theta)$, and let $f(x; \theta_0)$
be its evaluation at $\theta = \theta_0$, and we define $d$ as the 
dimension of $\theta$.
...
\]
\jy{What is $f$? Be careful with the dimension of $U$ and $f$.}
\mc{addressed}
...
as frequently. \jy{Watch proper usage of dashes.}\mc{addressed}
Still, even for these moderately dependent samples, the 
...
and $X_i \sim N(8, 8)$.
\jy{What's the logical connection of this paragraph with the
  surrounding paragraphs?}
\mc{highlighting that even samples with moderate positive temporal dependence
have block sizes larger than expected}
...
Despite the unexpected large block sizes, the
algorithm with \citet{politis2004automatic}'s setting holds its size somewhat 
well for moderately 
positively dependent
series as demonstrated in 
Figure~\ref{fig:alt_qq}. Both 
...
powerful for $|\tau| <= 0.5$, with \citet{politis2004automatic}'s
setting perhaps even having higher rejection rates for smaller negatively
dependent sample sizes. However, we still
\jy{too strong. soften it.}\mc{addressed}
caution using
\citet{politis2004automatic}'s setting because it fails to hold its size
for negative $\tau$, particularly for smaller sample sizes. 
