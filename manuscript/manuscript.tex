...
situations. Employing a parametric method would necessitate defining a
...
\section{Block Size Investigation}\label{sec:appendix}
...
Even for settings where the block sizes are usually less than $\sqrt{n}$ -
such as for $n = 800$, $\tau = 0.5$, and $X_i \sim N(8, 8)$ - the 
block
sizes are almost always larger than $l = \lceil n^{1/3} \rceil$.

\jy{How do you interpret the flat lines in the plot? Why are the
  empirical cdf step functions? That means your empirical p-values are
  discrete with point masses at a few points. Check the p-values from
  the simulations.}
\mc{this was a result of erroneous data, it is fixed now}
...
Despite this, our 
algorithm holds its size somewhat well for moderately dependent
series as demonstrated in 
Figures~\ref{fig:alt_normal} and \ref{fig:alt_gamma}. Both 
\citet{politis2004automatic}'s setting and the cubic root setting result
in the method not holding its size for $|\tau| > 0.5$. 
Figure~\ref{fig:alt_rr} also shows that the two settings are comparably
powerful for $|\tau| <= 0.5$. However, we still
recommend the cubic root setting over that introduced by 
\citet{politis2004automatic} because the latter fails to hold its size
for negative $\tau$, particularly for smaller sample sizes. It is important to 
note that another block size selection rationale could yield better results.

\begin{figure}[tbp]
  \centering
  \includegraphics[scale=1]{figures/alt_normal}
  \caption{Q-Q plots of the p-values testing that a time series
    have marginal normal distribution with true data generating distribution
    being $N(8,8)$, when using \citet{politis2004automatic}'s procedure
    to select block sizes.
  }
  \label{fig:alt_normal}
\end{figure}

\begin{figure}[tbp]
  \centering
  \includegraphics[scale=1]{figures/alt_gamma}
  \caption{Q-Q plots of the p-values testing that a time series
