...
radiation \citep{naess2012application}. \citep{aslam2020introducing} 
applied the method to test if the count rate per second of
radioactive data followed the Poisson distribution. 
Researchers have proposed the
method as a technique for gear condition monitoring \citep{andrade2001gear}. It
can also be used to compare images of breast
cancer tumors \citep{demidenko2004kolmogorov}.
...
bootstrap, where bootstrap samples of the test statistics are constructed from samples generated from the fitted hypothesized distribution.
...
distributions.
We generated time series $W_t$ from a 1st 
...
marginal $N(8, 8)$ by marginally transforming the $W_t$ by
\begin{equation*}
X_t = F^{-1}[\Phi(X_t)],
\end{equation*}
...
replacing $F^{-1}(p)$ with the quantile function for the unit exponential 
distribution.
...
the parametric bootstrap KS Test \citep{zeimbekakis2022misuses}) to
...
Annual temperature minimums were also recorded from Central Park, and the
same method comparison was applied to this series. We found that our method
yields a p-value of 0.0015, Babu's method yields a p-value of 0.2747, and
the parametric method yields a p-value of 0.0002.

\subsection{Temperature in Eastern Asia}
\label{sec:asia}

Annual maximums of daily maximum (TXx) and minimum (TNx) temperatures, and annual 
minimums of daily maximum (TXn) and minimum (TNn) temperatures were collected
for 60 years from 44 locations in Eastern Asia.
...
\citep{tseries}, 
we gathered closing stock prices for the SP500 index. We
...
\input{tables/sp500_pval_1y}

\input{tables/sp500_pval_2y}

\input{tables/sp500_pval_3y}

\input{tables/sp500_pval_4y}

\input{tables/sp500_pval_5y}
...
the series follows the $v = 5$ Student's $t$ distribution with p-value .064,
wheraeas \citep{babu2004goodness}'s method finds a p-value of .016, and 
\citet{zeimbekakis2022misuses}'s method finds a p-value of .01.
...
\citep{tseries}, 
we gathered closing stock prices for the SP100 index. We varied the
durations as we did with the SP500 index.
We approximated stock returns by taking the difference in the logarithm 
of the closing prices.
We tested that the stock returns are normally
distributed and Student's $t$ distributed 
with degrees of freedom $v \in \{30, 20, 10, 5, 4, 3, 2, 1\}$.
The p-values for these tests are highlighted in 
Tables~\ref{table:SP1001}--\ref{table:SP1005}.
For a duration of 4 years, our method fails to reject that
the series follows the $v = 5$ Student's $t$ distribution with p-value .092,
wheraeas \citep{babu2004goodness}'s method finds a p-value of .031, and 
\citet{zeimbekakis2022misuses}'s method finds a p-value of .027.

\input{tables/sp100_pval_1y}

\input{tables/sp100_pval_2y}

\input{tables/sp100_pval_3y}

\input{tables/sp100_pval_4y}

\input{tables/sp100_pval_5y}
...
demonstrated possible applications of this method to climate and financial data.
Although in many cases, the conclusions may not differ between our method
and other bootstrap KS tests, the p-values are different enough where in a case
where the temporal dependence in the data is more extreme, conclusions could
be drastically different if a different method is used.
