...
Let $H_0: X_t \sim F$, where $F$ is a hypothesized marginal distribution with 
unknown 
parameters. Let $H_A$ be that the marginal distribution of $X_t$ does not follow 
$F$. This is challenging 
because we are
dealing with a situation with both unknown parameters and serial dependence.
If $X_t \sim F$, the true marginal distribution of $X_t$ has some parameters 
$\theta$. The KS goodness-of-fit statistic is to test for the marginal 
distribution of a stationary series.
...
that the marginal distribution family is gamma.
...
Although the p-values do not appear to be completely aligned with the line
...
plots zoomed in to probabilities between 0 and 0.1, and as stated before, 
...
Like we did to see if the test holds its size, we generated time series with marginal distributions $N(8, 8)$ and 
$\Gamma(8, 1)$, lag-1 autocorrelation $\phi \in \{-.4, -.2, 0, .2, .4\}$, and
sample size $n \in \{100, 200, 400, 600, 800, 1600, 3200\}$. However, 
to evaluate the test's power, when $X_t \sim N(8, 8)$, we tested that the 
marginal distribution family is gamma, and when $X_t \sim \Gamma(8, 1)$, we tested
that the marginal distribution family is normal.
For each setting for $F$, $\phi$, and $n$, we replicate the method $R$ times 
to obtain a p-value $p_r$ for each $r \in \{1, \ldots, R\}$.

Given some significance level $\alpha$, we can compute a rejection rate 
$q = \#\{p_r < \alpha\} / R$ for $r \in \{1, \ldots, R\}$.
...
\input{tables/rr_1600}

\input{tables/rr_3200}

From Figure~\ref{table:rr_100}, we can see that when $n = 100$, the rejection rate may 
be quite low, especially for lower values of $\alpha$. From Figure~\ref{table:rr_200}, 
...
from Figure~\ref{table:rr_400} that the rejection rate
...
As shown in Figures~\ref{table:rr_800}, \ref{table:rr_1600}, and 
\ref{table:rr_3200}, the rejection rates for these settings are very 
...
follow the
Generalized Extreme Value (GEV) distribution. Precipitation maximum series 
...
We can test that the stock returns are normally
distributed (which we expect to reject).  After that, we can test that the
...
if it follows
the $t$ distribution with increasingly smaller specified degrees of freedoms. We
can also observe how the method performs using increasingly smaller durations of
time. First, we try the entire data set which has a duration of 25 years. Then,
we use the last 5 years of data: January 1st, 2018 to December 31st, 2022.
Finally, we try the method only using the last year of data: January 1st, 2022
