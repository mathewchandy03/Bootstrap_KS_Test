...
We will assume that the time series $\{X_t\}$ is stationary and strongly mixing, 
and $1/l+l/n\rightarrow 0$. We aim to show that under suitable assumptions
\begin{enumerate}
    \item $T_n \overset{d}{\rightarrow} \sup_x |G_{\infty}(x)+f(x;\theta_0)U|$ 
    for some Gaussian process $G_{\infty}(\cdot)$ and Gaussian random variable 
    $U$ defined below.
    \item $|\mathbb{P}(T_n^*\leq x|X_{1:n}) - 
    \mathbb{P}(T_n\leq x)|\overset{a.s.}{\rightarrow} 0$ for any fixed $x,$ 
    where $X_{1:n}:=\{X_1,\dots,X_n\}$ and $T_n^*$ is the the statistic computed 
    based on the bootstrap sample $X_1^*,\dots,X_n^*$). 
    Following the arguments in the proof of the Glivenko-Cantelli Theorem, we 
    may further strengthen this result to allow uniform convergence over $x$: 
    in other words, $\sup_x|\mathbb{P}(T_n^*\leq x|X_{1:n}) - 
    \mathbb{P}(T_n\leq x)|\overset{a.s.}{\rightarrow} 0$.
\end{enumerate}
Sketches of the proofs for these propositions are shown as supporting 
information.
...
\item
  %Fit $F_\theta$ to $X^{(b)}_1,...,X^{(b)}_n$ and obtain estimate 
	...
	$\hat\theta^{(b)}_n$ of $\theta$ from $X^{(b)}_1,...,X^{(b)}_n$.  
	\eds{changed this to remove $F_\theta$, please check}\mc{looks good}
\item
  Obtain the empirical distribution function $F^{(b)}_n$ of
  $X^{(b)}_1,...,X^{(b)}_n$. 
\item
...
\begin{table}[tbp]
\centering
\caption{Empirical sizes (ratio of rejection rate to significance level)
for the test that sample follows its true
	...
	\mc{addressed}
	\eds{I meant reporting the ratio, e.g, dividing all values currently in the    first row by 0.01, dividing all values in the second row by 0.05. Sometimes this
	is helpful if there is a conservative or liberal pattern, which there seems
	to be as $|\tau|$ increases away from zero, but it may be fine to leave it as
	is.}\mc{how does this look?}}
\label{table:size}
...
& $\tau = 0$ & $\tau = 0.25$ & $\tau = 0.5$ & $\tau = 0.75$ \\ 
  \midrule
{$N(8,8)$} & {100} & 0.01 & 5.210 & 1.240 & 1.120 & 1.060 & 1.140 & 1.580 & 5.630 \\ 
& & 0.05 & 2.940 & 1.256 & 1.090 & 1.096 & 1.150 & 1.450 & 3.410 \\ 
& & 0.10 & 2.277 & 1.186 & 1.147 & 1.072 & 1.154 & 1.424 & 2.714 \\ 
& {200} & 0.01 & 4.390 & 1.130 & 0.900 & 1.140 & 1.090 & 1.380 & 5.900 \\ 
& & 0.05 & 2.556 & 1.120 & 1.020 & 1.064 & 1.092 & 1.382 & 3.452 \\ 
& & 0.10 & 2.080 & 1.123 & 1.069 & 1.086 & 1.051 & 1.332 & 2.650 \\ 
& {400} & 0.01 & 3.240 & 1.070 & 0.990 & 1.080 & 0.970 & 1.260 & 4.670 \\ 
& & 0.05 & 2.032 & 1.000 & 0.994 & 1.120 & 0.978 & 1.184 & 2.836 \\ 
& & 0.10 & 1.734 & 1.023 & 1.002 & 1.089 & 1.017 & 1.192 & 2.232 \\ 
& {800} & 0.01 & 2.520 & 0.900 & 0.980 & 1.110 & 1.020 & 1.350 & 3.540 \\ 
& & 0.05 & 1.766 & 1.008 & 0.972 & 0.956 & 0.962 & 1.236 & 2.438 \\ 
& & 0.10 & 1.555 & 1.026 & 0.986 & 0.961 & 0.997 & 1.206 & 2.044 \\ [1ex]
{$\Gamma(8,1)$} & {100} & 0.01 & 4.880 & 1.420 & 1.170 & 1.240 & 1.110 & 1.610 & 5.760 \\ 
  & & 0.05 & 2.850 & 1.282 & 1.222 & 1.170 & 1.178 & 1.464 & 3.374 \\ 
  & & 0.10 & 2.241 & 1.224 & 1.147 & 1.175 & 1.150 & 1.375 & 2.679 \\ 
  & {200} & 0.01 & 4.080 & 1.000 & 0.890 & 1.220 & 1.160 & 1.430 & 6.080 \\ 
  & & 0.05 & 2.510 & 1.118 & 1.014 & 1.068 & 1.116 & 1.290 & 3.380 \\ 
  & & 0.10 & 1.991 & 1.105 & 1.022 & 1.048 & 1.072 & 1.233 & 2.664 \\ 
  & {400} & 0.01 & 3.140 & 0.970 & 1.140 & 0.940 & 1.030 & 1.310 & 4.300 \\ 
  & & 0.05 & 2.004 & 0.964 & 1.042 & 1.062 & 1.122 & 1.236 & 2.808 \\ 
  & & 0.10 & 1.697 & 1.025 & 1.066 & 1.018 & 1.100 & 1.177 & 2.232 \\ 
  & {800} & 0.01 & 2.490 & 0.920 & 1.130 & 1.120 & 1.000 & 1.130 & 3.810 \\ 
  & & 0.05 & 1.804 & 0.976 & 1.064 & 1.022 & 1.086 & 1.196 & 2.474 \\ 
  & & 0.10 & 1.561 & 1.024 & 1.072 & 1.034 & 1.067 & 1.177 & 1.997 \\ 
   \bottomrule
\end{tabular}
\end{table}
...
\eds{Is 1000 enough to truly evaluate whether size is good at the 0.01 level?}
...
A proof of our two propositions in \ref{sub:npbb} is provided.
