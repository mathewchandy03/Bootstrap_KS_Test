...
mysim <- function(n, phi, blksize, B, dist, f) {
  ...
  c(length(which(ks_values > obsv_ks)) / n)
}
...
```{r}
phi <- -0.4
(d_n_0.4 <- replicate(100, mysim(n, phi, blksize, B, dist, f)))
hist(d_n_0.4)
```
```{r}
phi <- -0.2
(d_n_0.2 <- replicate(100, mysim(n, phi, blksize, B, dist, f)))
hist(d_n_0.2)
```
```{r}
phi <- 0
(d_n_0 <- replicate(100, mysim(n, phi, blksize, B, dist, f)))
hist(d_n_0)
```
```{r}
phi <- 0.2
(d_0.2 <- replicate(100, mysim(n, phi, blksize, B, dist, f)))
hist(d_0.2)
```
```{r}
phi <- 0.4
(d_0.4 <- replicate(100, mysim(n, phi, blksize, B, dist, f)))
hist(d_0.4)
```
